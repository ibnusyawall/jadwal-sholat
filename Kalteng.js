...
var waktu = require('./inc/waktu.json'); // ini waktu nyavar req = require('request');
...
// wait
req('https://api.banghasan.com/sholat/format/json/jadwal/kota/852/tanggal/' + waktu.data[0].waktu, function (error, response, body){
   ambil = JSON.parse(body)
   console.log('------------------------------------------------'.hijau)
   console.log('|   '.hijau + 'Jadwal Sholat Provinsi '.biru + ini.data[2].v.biru + '   |'.hijau)
   console.log('------------------------------------------------'.hijau)
